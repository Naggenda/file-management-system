...
define("WEBSITE_BASE_PATH", "http://localhost/filemgt/dashboard/");
...
{
    if (in_folder()) {
        return create_url("index.php", ['id' => get_folder_id()]);
    } else {
        return create_url('index.php');
    }
}
...
function parent_up_url()
{
    ...
    if ($folder["parent"]) {
        return create_url("index.php", ['id' => $folder["parent"]]);
    } else {
        return create_url("index.php");
    }
}
...
<body>
    ...
    <form method="post" enctype="multipart/form-data" style="display: flex;">
        ...
        <label for="folder" class="form-label">Create Folder</label>
        <div class="form-group">
            <input class="form-input" type="text" name="folder">&nbsp;&nbsp;
        </div>
        <button class="btn blue" type="submit" name="submit">Submit</button>
    </form>

    <br><br>
    <h2><a href="<?Php echo create_url("index.php"); ?>">Files and Folders</a> <?php echo get_current_folder_name(); ?>
    </h2>
            ...
            <?php if (in_folder()): ?>
                <tr>
                    <td colspan="4"><img src='./images/ic_folder.png' style='width: 20px' /> &nbsp;
                        <a href="<?php echo parent_up_url(); ?>">...</a>
                    </td>
                </tr>
            <?php endif; ?>
            <?php foreach (get_all_parent_folders() as $folderObject): ?>
                <tr>
                    <td><img src='./images/ic_folder.png' style='width: 20px' /> &nbsp;
                        <a data-id="<?php echo $folderObject["id"]; ?>" data-type="folder"
                            data-filename="<?php echo $folderObject["name"]; ?>" class="item"
                            href="<?php echo create_url('index.php', ['id' => $folderObject["id"]]); ?>">
            ...
            <?php foreach (get_all_files() as $fObject): ?>
                <tr>
                    <td><?php echo get_icon($fObject["file_type"], $fObject["real_name"]); ?> &nbsp;
                        <a data-id="<?php echo $fObject["id"]; ?>" data-type="file"
                            data-filename="<?php echo $fObject["real_name"]; ?>" class="item" target="_blank"
                            href="<?php echo create_file_link($fObject["file_name"]); ?>">
        ...
        $(document).ready(function () {
            ...
            function rightClickEvents() {
                $.contextMenu({
                    ...
                    items: {
                        "rename": {
                            name: "Rename", icon: 'add', callback: function (key, opt) {
